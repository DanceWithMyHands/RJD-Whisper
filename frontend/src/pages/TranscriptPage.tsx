...
export default function TranscriptPage() {
  ...
  const taskSegmentIds = useMemo(
    () => new Set(meeting.assignments.map((a) => a.sourceSegmentId).filter(Boolean)),
    [meeting.assignments],
  )

  const filtered = meeting.transcript.filter((seg) => {
    const matchSpeaker = speakerFilter === 'all' || seg.speakerId === speakerFilter
      ...
      <Box>
        ...
        <Card><CardContent>
          <Stack alignItems="center" spacing={2} sx={{ py: 6 }}>
            <SubtitlesIcon sx={{ fontSize: 48, color: 'text.disabled' }} />
            <Typography color="text.secondary">Транскрипт пока не сформирован. Запишите совещание.</Typography>
            <Button variant="contained" onClick={() => navigate('/connect')}>Начать совещание</Button>
          </Stack>
        </CardContent></Card>
      </Box>
    )
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle={`${meeting.title} · распознано Whisper · ${meeting.transcript.length} реплик`}
        action={
          ...
          <Card>
            <CardContent>
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} sx={{ mb: 2 }}>
                ...
                />
                <ToggleButtonGroup
                  size="small" exclusive value={speakerFilter}
                  onChange={(_, v) => v && setSpeakerFilter(v)}
                  sx={{ flexShrink: 0 }}
                >
                  <ToggleButton value="all">Все</ToggleButton>
                  {meeting.participants.map((p) => (
                    <ToggleButton key={p.id} value={p.id}>
                      <Avatar sx={{ bgcolor: p.speakerColor, width: 22, height: 22, fontSize: 10 }}>
                        {initials(p.name)}
                      </Avatar>
                    </ToggleButton>
                  ))}
              ...
              <Stack spacing={2}>
                {filtered.map((seg) => {
                  const sp = participantById(seg.speakerId)
                  const isTask = taskSegmentIds.has(seg.id)
                  return (
                    <Stack key={seg.id} direction="row" spacing={1.5}>
                      <Tooltip title={sp?.name ?? ''}>
                        <Avatar sx={{ bgcolor: sp?.speakerColor, width: 38, height: 38, fontSize: 13, flexShrink: 0 }}>
                          {sp ? initials(sp.name) : '—'}
                        </Avatar>
                      </Tooltip>
                      <Box sx={{ flex: 1 }}>
                        <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 0.3 }}>
                          <Typography sx={{ fontWeight: 600, fontSize: 13.5 }}>{sp?.name}</Typography>
                          <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>
                            {formatDuration(seg.startSec)}
                          </Typography>
                          {isTask && (
                            <Chip size="small" color="primary" icon={<AssignmentTurnedInIcon />} label="Поручение"
                        ...
                        <Typography
                          sx={{
                            ...
                            borderLeft: isTask ? '3px solid' : 'none',
          ...
          <Card>
            <CardContent>
              ...
              <Typography sx={{ fontSize: 14, lineHeight: 1.6, color: 'text.secondary', mb: 2 }}>
                На совещании рассмотрены вопросы состояния путевого хозяйства, охраны труда,
                стабильности ИТ-систем мониторинга и логистики поставок. По итогам выделено{' '}
                <b>{meeting.assignments.length} поручения</b> с назначенными ответственными и сроками.
              </Typography>
              <Divider sx={{ mb: 2 }} />
              <Typography sx={{ fontWeight: 600, fontSize: 13.5, mb: 1 }}>Говорящие</Typography>
              <Stack spacing={1}>
                {meeting.participants.map((p) => {
                  const count = meeting.transcript.filter((s) => s.speakerId === p.id).length
                  return (
                    <Stack key={p.id} direction="row" spacing={1.5} alignItems="center">
                      <Avatar sx={{ bgcolor: p.speakerColor, width: 30, height: 30, fontSize: 12 }}>
                        {initials(p.name)}
                      </Avatar>
